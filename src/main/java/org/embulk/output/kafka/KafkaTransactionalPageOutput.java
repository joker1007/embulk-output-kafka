...
public abstract class KafkaTransactionalPageOutput<P, T extends P>
  ...
  public void add(Page page) {
    ...
    while (pageReader.nextRecord()) {
      ...
      String targetTopic =
          columnVisitor.getTopicName() != null ? columnVisitor.getTopicName() : topic;
      T record = columnVisitor.getRecord();

      ProducerRecord<Object, P> producerRecord =
          new ProducerRecord<>(targetTopic, columnVisitor.getPartition(), recordKey, record);
      producer.send(
          producerRecord,
          (metadata, exception) -> {
            if (exception != null) {
            ...
            logger.debug(
                "sent record: {topic: {}, key: {}, value: {}, partition: {}}",
            ...
            if (current >= recordLoggingCount.get()) {
              logger.info(
                  "[task-{}] Producer sent {} records", String.format("%04d", taskIndex), current);
