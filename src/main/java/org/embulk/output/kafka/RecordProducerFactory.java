...
class RecordProducerFactory {
  ...
  private static Properties buildProperties(
      KafkaOutputPlugin.PluginTask task, Schema schema, Map<String, String> configs) {
                  ...
                  new ColumnVisitor() {
                    ...
                    public void booleanColumn(Column column) {
                      throw new RuntimeException(
                          "boolean column is not supported for key_column_name");
                    }
                    ...
                    public void doubleColumn(Column column) {
                      kafkaProps.put(
                          ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, DoubleSerializer.class);
                    }
                    ...
                    public void timestampColumn(Column column) {
                      throw new RuntimeException(
                          "timestamp column is not supported for key_column_name");
                    }

                    @Override
                    public void jsonColumn(Column column) {
                      throw new RuntimeException(
                          "json column is not supported for key_column_name");
                    }
                  });
            });
  ...
  static KafkaProducer<Object, ObjectNode> getForJson(
      KafkaOutputPlugin.PluginTask task, Schema schema, Map<String, String> configs) {
    return new KafkaProducer<>(
        buildProperties(task, schema, configs), null, new KafkaJsonSerializer());
  }

  static KafkaProducer<Object, Object> getForAvroWithSchemaRegistry(
      KafkaOutputPlugin.PluginTask task, Schema schema, Map<String, String> configs) {
    KafkaAvroSerializer kafkaAvroSerializer = new KafkaAvroSerializer();
    String schemaRegistryUrl =
        task.getSchemaRegistryUrl()
            .orElseThrow(
                () ->
                    new ConfigException(
                        "avro_with_schema_registry format needs schema_registry_url"));

    Map<String, String> avroSerializerConfigs = new HashMap<>();
    avroSerializerConfigs.put(
