...
public class AvroFormatColumnVisitor extends KafkaOutputColumnVisitor<GenericRecord> {
  ...
  AvroFormatColumnVisitor(
      KafkaOutputPlugin.PluginTask task, PageReader pageReader, Schema avroSchema) {
  ...
  public void timestampColumn(Column column) {
    ...
    genericRecord.put(
        column.getName(), pageReader.getTimestamp(column).getInstant().toEpochMilli());
  }
  ...
  public void jsonColumn(Column column) {
    ...
    try {
      Object avroValue =
          convertMsgPackValueToAvroValue(avroSchema.getField(column.getName()).schema(), value);
  ...
  private Object convertMsgPackValueToAvroValue(org.apache.avro.Schema avroSchema, Value value) {
    switch (avroSchema.getType()) {
      case ARRAY:
        if (value.isArrayValue()) {
          return value.asArrayValue().list().stream()
              .map(item -> convertMsgPackValueToAvroValue(avroSchema.getElementType(), item))
              .filter(Objects::nonNull)
              .collect(Collectors.toList());
        }
        throw new RuntimeException(
            String.format(
                "Schema mismatch: avro: %s, msgpack: %s",
                avroSchema.getType().getName(), value.getValueType().name()));
      case MAP:
        if (value.isMapValue()) {
          ...
          for (Map.Entry<Value, Value> entry : value.asMapValue().entrySet()) {
            if (!entry.getValue().isNilValue()) {
              map.put(
                  entry.getKey().asStringValue().toString(),
                  convertMsgPackValueToAvroValue(avroSchema.getValueType(), entry.getValue()));
            }
          }
          return map;
        }
      ...
      case RECORD:
        if (value.isMapValue()) {
          ...
          for (org.apache.avro.Schema.Field field : avroSchema.getFields()) {
            Optional.ofNullable(valueMap.get(ValueFactory.newString(field.name())))
                .ifPresent(
                    v ->
                        record.put(
                            field.name(), convertMsgPackValueToAvroValue(field.schema(), v)));
          }
          return record;
        }
        throw new RuntimeException(
      ...
      default:
        throw new RuntimeException(
            String.format("Unsupported avro type %s", avroSchema.getType().getName()));
    }
  }
}
